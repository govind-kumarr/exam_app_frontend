...
import { ITextNode, textStyles } from "../types/interfaces/text-interfaces";
import { IEhancedTextField } from "../types/interfaces/component-interfaces";

const EnhancedTextField: FC<IEhancedTextField> = ({
  getTextNodes,
  error = false,
}) => {
  const [textNodes, setTextNodes] = useState<ITextNode[]>([]);
  const [showControls, setShowControls] = useState(false);
  const [activeControl, setActiveControl] = useState<textStyles>(
    STYLE_TYPES.NORMAL
  );
        ...
        {showControls &&
          textControls.map((control) => {
            ...
            return (
              ...
              >
                <IconButton
                  onClick={() => handleActiveControlClick(label)}
                  sx={{ p: 0.5 }}
                >
                  {icon}
                </IconButton>
